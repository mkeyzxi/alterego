...
export default function HabitDashboard({
  ...
  const completedCount =
    (habits.sleep ? 1 : 0) + (habits.water === 8 ? 1 : 0) + (habits.walk ? 1 : 0)
  ...
  const questPanelContent = (
    <div className="glass-panel" style={{padding: '20px'}}>
      ...
      >
        ...
        {/* Close button — mobile only (rendered inside drawer) */}
        <button
      ...
      <div className="flex flex-col gap-2.5">
          ...
          return (
            <motion.button
              ...
              style={{
                ...
                padding: '12px 14px',
                borderRadius: '12px',
                border: `1px solid ${item.checked ? `${item.color}55` : 'rgba(255,255,255,0.08)'}`,
                background: item.checked ? `${item.color}15` : 'rgba(255,255,255,0.03)',
            ...
            >
              <div
                style={{
                  ...
                  background: item.checked ? `${item.color}30` : 'rgba(255,255,255,0.05)',
                  flexShrink: 0,
                  transition: 'all 0.3s ease',
                }}
              >
        ...
        <motion.div
          ...
          onClick={() => {
            if (!questCompleted && habits.water < 8) {
              onAddWater()
            }
          }}
          role="button"
          ...
          style={{
            ...
            padding: '12px 14px',
            borderRadius: '12px',
            border: `1px solid ${habits.water === 8 ? '#38bdf855' : 'rgba(255,255,255,0.08)'}`,
            background: habits.water === 8 ? '#38bdf815' : 'rgba(255,255,255,0.03)',
        ...
        >
          ...
          <div
            style={{
              ...
              alignItems: 'center',
              justifyContent: 'center',
              background: habits.water === 8 ? '#38bdf830' : 'rgba(255,255,255,0.05)',
              flexShrink: 0,
              transition: 'all 0.3s ease',
            }}
          >
          ...
          <div style={{flex: 1, minWidth: 0}}>
            ...
            >
              ...
              <span
                style={{
                  ...
                  background:
                    habits.water === 8 ? 'rgba(56, 189, 248, 0.15)' : 'rgba(255,255,255,0.08)',
                  padding: '2px 10px',
                  borderRadius: '8px',
                }}
              >
                {habits.water}/8 <GlassWater />
              </span>
            </div>
            <div
              style={{
            ...
            >
              ...
              {habits.water < 8 && !questCompleted && <Plus size={14} style={{color: '#38bdf8'}} />}
            </div>
          </div>
        ...
        {questCompleted && (
          <motion.div
            ...
            style={{
              ...
              padding: '10px 14px',
              borderRadius: '10px',
              background: isOptimal ? 'rgba(16, 185, 129, 0.15)' : 'rgba(239, 68, 68, 0.15)',
              border: `1px solid ${
                isOptimal ? 'rgba(16, 185, 129, 0.3)' : 'rgba(239, 68, 68, 0.3)'
              }`,
              fontSize: '12px',
              fontWeight: 600,
          ...
          >
            {isOptimal ? 'Quest Selesai! Duniamu bersinar!' : 'Quest Selesai. Duniamu retak...'}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
  ...
  return (
    <>
      {/* ===== MOBILE: Floating "Buka Quest" button (visible when drawer closed) ===== */}
      <AnimatePresence>
        {!isQuestOpen && (
          <motion.button
            id="open-quest-btn"
            ...
            whileTap={{scale: 0.95}}
            className="md:!hidden flex fixed top-45 left-1/2 -translate-x-1/2 z-40 items-center gap-2"
            style={{
              padding: '14px 28px',
              borderRadius: '16px',
              border: 'none',
      ...
      {/* ===== MOBILE: Bottom Sheet Drawer Overlay (z-50) ===== */}
      <AnimatePresence>
        {isQuestOpen && (
          <>
            {/* Backdrop overlay */}
            ...
            {/* Drawer panel sliding up from bottom */}
            <motion.div
              initial={{y: '100%'}}
              animate={{y: 0}}
              exit={{y: '100%'}}
      ...
      {/* ===== DESKTOP: Always-visible side panel (z-40) ===== */}
      <motion.div
        initial={{opacity: 0, y: 20}}
        animate={{opacity: 1, y: 0}}
        ...
        className={`
          hidden md:block
          absolute top-5 right-5 w-80 z-40
          ${isCritical ? 'animate-glitch' : ''}
        `}
      >
        {questPanelContent}
